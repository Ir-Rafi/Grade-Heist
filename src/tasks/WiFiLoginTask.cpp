...
, credIndex(0)
{
    ...
    font.loadFromFile("assets/arial.ttf"); // fallback if it fails
...
void WiFiLoginTask::generateCredentials() {
    ...
    //     "CSEDU_Student", "CSEDU_Admin",
    //     "WiFi_User",     "Campus_Guest",
    //     "Network_Member"
    // };
    ...
    // // build exactly five entries, in order, no randomness
    // for (int i = 0; i < 5; ++i) {
    //     WiFiCredentials c;
    //     c.username = users[i] + std::to_string(i+1);
    //     // cycle through prefs/sufs deterministically
    //     c.password = prefs[i % prefs.size()]
    //                + sufs[i % sufs.size()];
    //     creds.push_back(c);
    // }
    WiFiCredentials c;

    c.username = "CSEDU_Admin";
    c.password = "Access1234!";
