...
sf::FloatRect createLineAABB(const sf::Vector2f& p1,
...
{
    // A) Range check: if enemy ≥100 units away, never end the game
    if ( dist(pos_enemy, pos_player) >= 100.f )
        return false;

    bool ok = false;

    // B) Obstacle check: build a thin AABB “line” between enemy and player
    for (auto& w : walls) {
        sf::FloatRect line = createLineAABB(pos_enemy, pos_player, 2.f);
        if ( line.intersects( w.getGlobalBounds() ) )
            ok = true;    // ← *this* returns true if the line *does* hit a wall
    }

    // C) If no wall ever intersects, we fall through and return false
    return (!ok);
}
